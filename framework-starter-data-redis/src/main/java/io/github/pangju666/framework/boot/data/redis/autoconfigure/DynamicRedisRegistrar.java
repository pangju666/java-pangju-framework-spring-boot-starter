...
import io.github.pangju666.framework.boot.data.redis.DynamicRedisUtils;
...
class DynamicRedisRegistrar implements EnvironmentAware, BeanFactoryAware, ImportBeanDefinitionRegistrar {
	...
	public void registerBeanDefinitions(AnnotationMetadata importingClassMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
		DynamicDataRedisProperties dynamicRedisProperties;

		try {
			dynamicRedisProperties = binder.bind(DynamicDataRedisProperties.PREFIX,
				DynamicDataRedisProperties.class).get();
		...
		Map<String, DynamicDataRedisProperties.DataRedisProperties> redisDatabases = dynamicRedisProperties.getDatabases();
		if (!CollectionUtils.isEmpty(redisDatabases)) {
			redisDatabases.forEach((name, redisProperties) -> {
				...
				BeanDefinitionBuilder connectionDetailsBeanBuilder = BeanDefinitionBuilder.genericBeanDefinition(
					DataRedisConnectionDetails.class, connectionDetailsSupplier);
				...
				if (redisProperties.getClientType() == DynamicDataRedisProperties.DataRedisProperties.ClientType.JEDIS) {
					Supplier<JedisConnectionFactory> connectionFactorySupplier = () -> {
						...
						JedisConnectionConfiguration connectionConfiguration = new JedisConnectionConfiguration(redisProperties,
							...
							beanFactory.getBeanProvider(RedisStaticMasterReplicaConfiguration.class),
							beanFactory.getBean(connectionDetailsBeanName, DataRedisConnectionDetails.class));
