...
public class OnDynamicRedisLettuceCondition extends SpringBootCondition {
    ...
    public ConditionOutcome getMatchOutcome(ConditionContext context, AnnotatedTypeMetadata metadata) {
		...
		if (bindResult.isBound()) {
			DynamicDataRedisProperties properties = bindResult.get();
			if (StringUtils.hasText(properties.getPrimary()) &&
				...
				for (DynamicDataRedisProperties.DataRedisProperties value : properties.getDatabases().values()) {
					if (Objects.isNull(value.getClientType()) ||
						value.getClientType() == DynamicDataRedisProperties.DataRedisProperties.ClientType.LETTUCE) {
