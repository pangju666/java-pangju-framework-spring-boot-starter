...
class GMImageTemplateSpec extends Specification {
	@Autowired
	GMImageTemplate template
	...
	def "测试图片缩放: 目标宽 #targetW"() {
		...
		and: "构建缩放操作配置"
		def operation = ImageOperationBuilders.gm()
	...
	def "测试图片偏移裁剪"() {
		...
		"test.webp"   | "test.webp"    | 550   | 368    | 100 | 100    | 300  | 100
	}
	...
	def "测试旋转"() {
		...
		"test.webp"   | "test.webp"    | 550   | 368    | RotateDirection.CLOCKWISE_90
	}
	...
	def "测试组合操作: 裁剪 -> 缩放 -> 旋转 -> 翻转 -> 灰度化 -> 亮度 -> 对比度 -> 锐化 -> 模糊 -> 图像水印 -> 转格式"() {
		...
		def genericOperation = ImageOperationBuilders.generic()
			...
			.watermarkImage(new ClassPathResource("images/watermark.png").getFile())
			.build()
		def bufferedOperation = ImageOperationBuilders.gm(genericOperation)
