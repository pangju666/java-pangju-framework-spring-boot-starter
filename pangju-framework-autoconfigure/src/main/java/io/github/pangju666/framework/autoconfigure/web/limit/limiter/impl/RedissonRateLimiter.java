...
public class RedissonRateLimiter implements RateLimiter {
	 ...
	 * 限流配置属性
	 * <p>
	 * 包含限流器的配置信息，如Redis键前缀和Redisson客户端Bean名称
	 * </p>
	 *
	 * @since 1.0.0
	 */
	 ...
	 * 构造方法
	 * <p>
	 * 初始化Redisson限流器，根据配置获取RedissonClient实例。
	 * </p>
	 * <p>
	 * Redisson客户端获取优先级：
	 * <ol>
	 *     <li>如果配置了redisson-client-bean-name，使用指定的Bean</li>
	 *     <li>否则使用容器中的默认RedissonClient Bean</li>
	 * </ol>
	 * </p>
	 *
	 * @param properties 限流配置属性，包含Redis相关配置
	 * @param beanFactory Spring Bean工厂，用于获取RedissonClient Bean
	 * @since 1.0.0
	 */
	public RedissonRateLimiter(RateLimitProperties properties, BeanFactory beanFactory) {
		this.properties = properties;
		if (StringUtils.isNotBlank(properties.getRedisson().getRedissonClientBeanName())) {
			this.redissonClient = beanFactory.getBean(properties.getRedisson().getRedissonClientBeanName(), RedissonClient.class);
		} else {
