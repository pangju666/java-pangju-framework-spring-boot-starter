...
package io.github.pangju666.framework.boot.jackson.crypto.serializer;
...
import io.github.pangju666.framework.boot.jackson.crypto.annotation.EncryptFormat;
 ...
 * @since 1.0.0
 */
public final class EncryptJsonSerializer extends ValueSerializer<Object> {
	/**
	 * 日志记录器
	 *
	 * @since 1.0.0
	 ...
	 * @since 1.0.0
	 */
	private void writeBytes(byte[] value, JsonGenerator gen) {
	 ...
	 * @since 1.0.0
	 */
	private void writeString(CharSequence value, JsonGenerator gen) {
	...
	/**
	 * 加密 {@link BigInteger} 并写入 JSON 字符串
	 * <p>
	 ...
	 * @since 1.0.0
	 */
	private void writeBigInteger(BigInteger value, JsonGenerator gen) {
	...
	/**
	 * 加密 {@link BigDecimal} 并写入 JSON 字符串
	 * <p>
	 * 当值为 null 时输出 JSON null；非空时加密后写入数值的字符串表示。
	 * </p>
	 *
	 ...
	 * @since 1.0.0
	 */
	private void writeBigDecimal(BigDecimal value, JsonGenerator gen) {
		if (Objects.isNull(value)) {
			gen.writeNull();
		} else {
	 ...
	 * @since 1.0.0
	 */
	private void writeIterable(Iterable<?> values, JsonGenerator gen) {
		gen.writeStartArray();
	...
	private void writeMap(Map<?, ?> value, JsonGenerator gen) {
		...
		for (var entry : value.entrySet()) {
			if (entry.getValue() instanceof byte[] bytes) {
				gen.writeName(Objects.toString(entry.getKey()));
				writeBytes(bytes, gen);
				...
				gen.writePOJOProperty(Objects.toString(entry.getKey()), entry.getValue());
			}
		}
		gen.writeEndObject();
	}
