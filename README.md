...
- **🛡️ 安全加/解密**：原生支持请求/响应加解密、JSON 字段脱敏与加密，支持多种加解密算法（AES, RSA 等）。
- **📊 深度集成**：与 MyBatis Plus, Redis, MongoDB 深度集成，提供统一的异常处理、分页增强和多数据源支持。
- **📸 图像处理**：支持多种图像引擎（ImageIO, OpenCv, GraphicsMagick），提供裁剪、缩放、格式转换等统一接口。
- **🗜️ 压缩归档**：支持多种压缩算法（GZIP、XZ、Zstd）和归档格式（TAR、7-Zip、ZIP），提供统一的压缩归档接口。
- **🔍 文字识别**：支持 Tesseract API 和 Tesseract CLI 两种 OCR 引擎，提供图像文字识别的统一接口。
...
| `framework-starter-compress`          | Framework 归档/加密归档/压缩 Starter                   |
| `framework-starter-ocr`               | Framework 图像OCR Starter                              |
